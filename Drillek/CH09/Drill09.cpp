#include "std_lib_facilities.h"

namespace UDChrono{

class Year{
	static constexpr int min = 1800;
	static constexpr int max = 2077;
public:
	class Invalid{};
	//melyik ev es atadjuk egy lokalis valtozonak
	Year(int x): y(x) { if( x < min || x > max ) throw Invalid{}; }
	int year() { return y; }
	void increment() { y++; }
private:
	int y;
};

Year operator++(Year& year)
{
	year.increment();
}

ostream& operator<<(ostream& os, Year year)
{
	return os << year.year();
}

const vector<string> months = 
{
	"January",
	"February",
	"March",
	"April",
	"May",
	"June",
	"July",
	"August",
	"September",
	"October",
	"November",
	"December"
};

enum class Month {
	jan, feb, mar, apr, may, june, july, aug, sept, oct, nov, dec
};

Month operator++(Month& m)
{
	//month egyenlo dec  ha igen ez fut  ellenben ez
	m = (m==Month::dec) ? Month::jan : Month(int(m)+1);
	return m;
}


//ezzl mukodtetjuk a monthra '<<'
ostream& operator<<(ostream& os, Month m)
{
	return os << months[int(m)];
}

class Date
{
	// ev, honap, nap
	Year year;
	Month month; 
	int day;

public:
	class Invalid {};
	Date(): year(Year{2001}), month(Month::jan), day(1) {}
	//konstruktor "Date"-ok hatarozzak meg 1-1 objektum hogy keszul el
	//							year tagnak ertekul adjuk y stb...
																//true negalva false, nemdobja invalid ellenkezo igen
	Date(Year y, Month m, int d): year(y), month(m), day(d) { if(!is_valid()) throw Invalid {}; }

	bool is_valid();
	void add_day(int n);
	Year get_year() const { return year; }
	Month get_month() const { return month; }
	int get_day() const { return day; }
};

bool Date::is_valid()
{
	//ha nem jo
	if(day < 1 || day > 31)
		return false;

	return true;
}

/*
//helper fuggveny datum inicializalas
Date::Date(int y, int m, int d)
{
	if(y > 0)
		year=y; 
	else
		error("Invalid year");
	if(m <= 12 && m > 0)
		month=m; 
	else
		error("Invalid month");
	if(d > 0 && d <= 31)
		day = d; 
	else
		 error("Invalid day");
}
*/

//datum leptetes/hozzaadas
void Date::add_day(int n)
{
	//hogyha a nap >31 noveljuk honapot es ha honap >12 noveljuk evet is
	day += n;
	if(day > 31)
	{
		++month;
		day -=31;
		if(month == Month::jan)
		{
			++year;
		}
	}
}

} //UDCHrono end!

int main()
try {

	using namespace UDChrono;
	
	Date today {Year{2077}, Month::dec, 31};

	today.add_day(1);

	cout << "Date: " << today.get_year() << ". "
		<< today.get_month() << ' ' << today.get_day() << ".\n";
		//HONAP tipusus operator '<<'

	//az ertekek csak siman lemasolodnak
	Date tomorrow {today};
	cout << "Date: " << tomorrow.get_year() << ". "
		<< tomorrow.get_month() << ' ' << tomorrow.get_day() << ".\n";

	vector<Date> dates(10);

	cout << "Vector\n";
	for(const auto& date : dates)
		cout << "Date: " << date.get_year() << ". "
		<< date.get_month() << ' ' << date.get_day() << ".\n";


	cout << "\n";
	Date my_birthday {Year{2020},Month::dec,31};

	cout << "Date: " << my_birthday.get_year() << ". " << my_birthday.get_month() << " " << my_birthday.get_day() << ".\n";

	my_birthday.add_day(1);

	cout << "Date: " << my_birthday.get_year() << ". " << my_birthday.get_month() << " " << my_birthday.get_day() << ".\n";

	Date other_date {my_birthday};

	cout << "Date: " << other_date.get_year() << ". " << other_date.get_month() << " " << other_date.get_day() << ".\n";

	Date def_date;

	cout << "Date: " << def_date.get_year() << ". " << def_date.get_month() << " " << def_date.get_day() << ".\n";


	return 0;

}catch(UDChrono::Date::Invalid){
	cout << "Invalid date\n";
	return 1;
}catch(UDChrono::Year::Invalid){
	cout << "Invalid year\n";
	return 1;
}catch(exception& e){	
	cout << e.what() << endl;
	return 1;
}